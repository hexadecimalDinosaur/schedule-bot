...
async def on_message(message):
    ...
    if message.content.lower().startswith('$ping'):
        ...
        elif str(message.author.id) not in data[str(message.channel.guild.id)]['users'].keys() or str(content[1].upper()) not in set(data[str(message.channel.guild.id)]['users'][str(message.author.id)]['courses']):
        ...
        if len(content) < 2:
            ...
            await message.channel.send("This class does not exit. Contact your admin to add any new courses.")
        elif str(message.author.id) not in data[str(message.channel.guild.id)]['users'].keys() or str(content[1].upper()) not in data[str(message.channel.guild.id)]['users'][str(message.author.id)]['courses']:
            await message.channel.send("You are not in this class")
        else:
            if 'events' not in set(data[str(message.channel.guild.id)]['courses'][content[1].upper()].keys()):
                data[str(message.channel.guild.id)]['courses'][content[1].upper()]['events'] = []
